...
# Fuction, getting file's size in the convinient format
...
# Main function for comprassions of the image
def compress_img(image_name, new_size_ratio = 0.9,quality = 90, width = None, height = None, to_jpg = True):
    ...
    if new_size_ratio < 1.0:
        # if resizing ratio below 1.0, then multiply width and height with this ratio to reduce image size
    ...
    # make new filename appending_compressed to the original file name
    if to_jpg:
        # change the extention to JPEG
        new_filename = f"{filename}_compressed.jpg"
    else:
        # retain the same extention of the original image
        new_filename = f"{filename}_compressed{ext}"
    try:
        # save the image with corresponding quality and optimize set to True
        img.save(new_filename, quality=quality, optimize=True)
    except OSError:
        # convert the image to RGB mode first
        img = img.convert("RGB")
        # save the image with the corresponding quality and optimaze set to True
        img.save(new_filename, quality=quality, optimize=True)
...
if __name__ == "__main__":
    import argparse
    parser = argparse.ArgumentParser(description="Simple Python script for compressing and resuming images")
    parser.add_argument("image", help="Target image to compress and/or resize")
    parser.add_argument("-j", "--to-jpg", action="store_true", help="Whether to convert the image to the JPEG fromat")
